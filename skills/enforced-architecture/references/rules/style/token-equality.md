...
[`style/token-equality.ts`](token-equality.ts), behind the structural orchestrator. It returns findings and never prints or exits; reporting and the exit code belong to `scripts/run-structural-checks.ts`.
