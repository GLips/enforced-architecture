...
//
// See structure/layer-direction.md for the negative space and the adapt notes,
// and graph/import-graph.md for how an edge gets resolved at all.
//
// ──────────────────────────────────────────────────────────────────────

import type { Finding, StructuralCheck } from "../scripts/lib.ts";
...
export const layerDirectionCheck: StructuralCheck = {
  ...
  run({ config, importGraph }) {
    ...
    for (const edge of importGraph()) {
      ...
      // at all is `structure/topology`'s finding rather than this one's.
