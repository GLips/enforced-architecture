...
Every rule here matches an import path, so a relative import reaching the same module by a different spelling bypasses all of them at once. Adopt `cross-boundary-alias` alongside the first path-matching rule you take.
...
| [env-access](env-access.ts) | oxlint | Yes | Any module but the env module reading `process.env` directly |
