...
| Rule | Mechanism | Blocking | What it prevents | Tag if not Effect |
|---|---|---|---|---|
| [no-disable-validation](no-disable-validation.ts) | oxlint | Yes | `{ disableValidation: true }` on a Schema constructor — and the shorthand, quoted, computed, and forwarded-flag spellings of it | structure |
| [no-silent-error-swallow](no-silent-error-swallow.ts) | oxlint | Yes | A catch handler whose body is `Effect.void`, in either the data-first or the data-last spelling, including the `catchTags` object form | testing |
| [no-effect-catchallcause](no-effect-catchallcause.ts) | oxlint | Yes | `Effect.catchAllCause` and `Effect.catchAllDefect` — handling a bug as if it were an expected failure | boundary |
| [no-service-option](no-service-option.ts) | oxlint | Yes | `Effect.serviceOption`, which turns a missing layer from a compile error into a quiet runtime `None` | boundary |
| [no-nested-layer-provide](no-nested-layer-provide.ts) | oxlint | Yes | `Layer.provide` nested anywhere inside another `Layer.provide`'s arguments, including through `.pipe(…)` | health |
| [no-sql-type-parameter](no-sql-type-parameter.ts) | oxlint | Yes | ``sql<Row>`…` `` — a row shape declared and never checked. Use `SqlSchema` with a `Schema` | types |

Two of them are one catalog idea in Effect clothing:

- **`no-disable-validation` is the opt-out half of a mandate.** Any rule requiring validation is satisfied by a validator call that validates nothing, so the mandate and the ban on its no-op form have to ship together — `structure/server-fn-validation` is the mandate half at a different boundary.
