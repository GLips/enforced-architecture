...
2. **Extract imports** -- `Bun.Transpiler.scanImports`, never a pattern over the source. It returns what survives to runtime, so type-only imports — which are erased and cannot pull in server-only code — are already absent.
...
- **Import extraction** uses `Bun.Transpiler.scanImports`, per [Extraction](../graph/import-graph.md#extraction) — not a pattern over the source, which cannot tell a real import from a code sample in a template literal. Read that section for the loader and shebang traps, both of which abort rather than degrade.
  ...
  **Prefer `scan()` here, not `scanImports()`.** Under a JSX loader `scanImports` returns the JSX runtime imports Bun injects — `react/jsx-dev-runtime` and a duplicate `react`, both tagged `require-call` and so indistinguishable from real ones. This rule's subject is bare package names checked against a blocklist, which is exactly where an injected path can produce a violation on a file that imports no such thing. `scan()` filters them and returns exports too. See [It reports imports that are not in the file](../graph/import-graph.md#it-reports-imports-that-are-not-in-the-file).
