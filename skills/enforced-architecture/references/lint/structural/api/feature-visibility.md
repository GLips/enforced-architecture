...
- **A grant that goes stale by deletion is reported in CI only.** The orchestrator suppresses warnings for files a commit did not stage, and every finding here is filed against the importee's `visibility.json`. That is fine for the two warnings a commit *writes* — a grant naming a nonexistent feature, or one added a commit ahead of its import, both come from editing that very file, so both surface at pre-commit. It is not fine for the one nobody writes: a grant goes stale when the importer deletes its last import, and that commit stages a file in the *importing* feature and has no reason to open the importee's. So the specific case this warning exists for — coupling that can return with no diff — reports on an unfiltered run and not in the hook. Run the tier in CI as well as pre-commit, or that case is one you do not cover. Errors are never filtered, which is separately what makes filing a blocking finding against a file the offending commit never touched work at all.
