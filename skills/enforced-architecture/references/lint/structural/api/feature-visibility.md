...
A feature here is a **directory** under `features/`, occupied or not. That is wider than the rest of the tier, which walks directories holding source, and it is the point: a leftover directory whose code is gone but whose `visibility.json` is not is where a grant outlives its whole feature. The importee end of an edge is resolved against the tree the same way, so a name that reaches real code reaches a grant file — a symlinked feature, or a case-mismatched specifier on a case-insensitive filesystem, is the same feature as the directory it lands on and needs the same grant.
