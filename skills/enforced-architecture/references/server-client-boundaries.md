...
The compiler's extraction has known edge cases around wrappers and abstractions. If a controller triggers build errors from server-only imports leaking into the client bundle, split into two files with dynamic `await import()`:
